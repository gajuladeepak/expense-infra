resource "aws_cloudfront_distribution" "expense" {
  ...
  ordered_cache_behavior {
    path_pattern     = "/images/*"
    allowed_methods  = ["GET", "HEAD", "OPTIONS"] #allowing these methods
    cached_methods   = ["GET", "HEAD", "OPTIONS"] #caching these methods
    target_origin_id = "${var.project_name}-${var.environment}.${var.zone_name}"
  ...
  #with this we are enabling the URLs in few countries
  # with this the website will not open in other countries other than ["IN", "CA", "GB", "DE"]
  restrictions {
    geo_restriction {
  ...
  viewer_certificate {
    ...
    ssl_support_method = "sni-only" #this line is requried if we give acm certificate
